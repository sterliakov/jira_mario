...
const App = () => {
    ...
    const [isProjectConfigSubmitted, setProjectConfigSubmitted] =
        useState(false);
    ...
    const isToggleConfigSelected = (name) =>
        projectConfigState &&
        projectConfigState[name] && {defaultChecked: true};
    ...
    const renderDateTimeOption = (option) => {
        ...
        switch (option) {
            case DATE_TIME_OPTIONS.day:
                label = `Day-month-year: ${format(
                    sampleDate,
                    DATE_TIME_OPTIONS.day,
                )}`;
                break;
        ...
        return projectData.length !== 0 ? (
            <Fragment>
                <Text>
                    ...
                    <Strong>Issue health monitor</Strong> configuration for
                    selected project
                </Text>
    ...
    const renderHistoricalAssigneeConfig = () => (
        <Toggle
            {...isToggleConfigSelected('isHistoricalAssigneeVisible')}
